...
def create_app():
    ...
    @app.route('/api/health/connect-probe')
    @limiter.limit('4 per minute')
    def health_connect_probe():
        """Test-mode diagnostic: for ?garden_id=, run the exact Connect
        onboarding calls the billing page makes (account retrieve + embedded
        AccountSession + hosted AccountLink) and report Stripe's real errors.
        No secrets; refuses on live keys."""
        from app import stripe_service
        ...
        if key.startswith(('sk_live', 'rk_live')):
            return jsonify({'error': 'disabled in live mode'}), 403
        gid = flask_request.args.get('garden_id', type=int)
        garden = CommunityGarden.query.get(gid) if gid else None
        if not garden or not garden.organizer:
            return jsonify({'error': 'garden or organizer not found'}), 404
        org = garden.organizer
        _stripe.api_key = key
        out = {'garden_id': gid, 'account_id': org.stripe_connect_account_id}
        try:
            if org.stripe_connect_account_id:
                acct = _stripe.Account.retrieve(org.stripe_connect_account_id)
                out['charges_enabled'] = acct.charges_enabled
                out['payouts_enabled'] = acct.payouts_enabled
                req = acct.requirements or {}
                out['disabled_reason'] = req.get('disabled_reason')
                out['currently_due'] = list(req.get('currently_due') or [])[:10]
        except Exception as e:
            out['account_error'] = f'{type(e).__name__}: {e}'[:300]
        try:
            stripe_service.create_account_session(org)
            out['account_session'] = 'ok'
        except Exception as e:
            out['account_session_error'] = f'{type(e).__name__}: {e}'[:300]
        try:
            stripe_service.create_connect_account_link(org, return_path=f'/gardens/{gid}/billing')
            out['account_link'] = 'ok'
        except Exception as e:
            out['account_link_error'] = f'{type(e).__name__}: {e}'[:300]
        return jsonify(out)
